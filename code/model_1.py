...
class AblationGCNNetmuti(torch.nn.Module):
    def __init__(self, ablation_mode='baseline', n_output=1, n_filters=32, embed_dim=64, num_features_xd=78,
                 num_features_smile=66, num_features_xt=25, output_dim=128, dropout=0.2):
        super(AblationGCNNetmuti, self).__init__()
        ...
        # miRNA structure matrix branch (不变)
        if ablation_mode in ['baseline', 'attn_fusion', 'no_sequence']:
        ...
        # Attention fusion for miRNA
        if ablation_mode == 'attn_fusion':
        ...
        # Linear fusion for miRNA (不变)
        if ablation_mode == 'baseline':
            self.fusion_mirna = nn.Linear(output_dim * 2, output_dim)
    ...
    def process_drug_fingerprints(self, rdkit_descriptor, rdkit_fingerprint, maccs_fingerprint, morgan_fingerprint):
        ...
        assert rdkit_descriptor.shape[-1] == self.rdkit_descriptor_dim, "rdkit_descriptor dim mismatch"
    ...
    def forward(self, data):
        ...
        try:
            ...
            print(f"[ERROR] Reshape failed for fingerprints")
            raise e
        ...
        conv_xt = None
        if self.ablation_mode != 'no_sequence':
            try:
        ...
        matrix_feat = None
        if self.ablation_mode in ['baseline', 'attn_fusion', 'no_sequence']:
            if target_matrix is None: raise ValueError("target_matrix is required")
        ...
        # ============= miRNA Fusion =============
        if self.ablation_mode == 'no_matrix':
            mirna_features = conv_xt
        elif self.ablation_mode == 'no_sequence':
            mirna_features = matrix_feat
        elif self.ablation_mode == 'attn_fusion':
            ...
            mirna_features = self.fusion_mirna(mirna_concat)
            mirna_features = self.relu(mirna_features)  # LeakyReLU
