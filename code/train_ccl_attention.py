...
# ============================================================================
# 可选模式: 
# 'BASELINE' : 原始模型 (Attn + CCL)
# 'NO_ATTN'  : 去除交叉注意力 (No Attn + CCL)
# 'NO_CCL'   : 去除对比学习 (Attn + No CCL)

ABLATION_MODE = 'NO_CCL'  # <--- 在这里修改你想跑的实验
...
if ABLATION_MODE == 'BASELINE':
    ...
    USE_ATTENTION = False
    USE_CCL = True
elif ABLATION_MODE == 'NO_CCL':
    USE_ATTENTION = True
    USE_CCL = False
else:
    raise ValueError(f"Unknown mode: {ABLATION_MODE}")

print(f"{'='*70}")
print(f"[Ablation] Current Mode: {ABLATION_MODE}")
print(f"[Config] Use Attention: {USE_ATTENTION}")
print(f"[Config] Use Contrastive Learning (CCL): {USE_CCL}")
print(f"{'='*70}\n")
...
def predicting(model, device, loader):
    ...
    with torch.no_grad():
        for data in loader:
            data = data.to(device)
            # 预测时不管用不用 CCL，return_contrastive_loss 设为 False 即可
            output, _ = model(
                data,
                current_epoch=0,
                total_epochs=NUM_EPOCHS,
...
if __name__ == "__main__":
    ...
    for fold in range(NUM_FOLDS):
        print(f"\n{'='*70}")
        print(f">>> Fold {fold + 1}/{NUM_FOLDS} | Mode: {ABLATION_MODE}")
        print(f"{'='*70}")
    ...
    print("\n" + "="*80)
    print(f"FINAL 5-FOLD CV RESULTS | Mode: {ABLATION_MODE}")
    print("="*80)
